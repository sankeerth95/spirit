...
void Text::generateTextMap(float x, float y, float size) {
    ...
    for(unsigned int i = 0; i < message.length(); i++){
        ...
        v1.tex_coords = glm::vec2((message[i]%16)/16.0f, (message[i]/16)/16.0f);

        v2.vertices = glm::vec4(x+i*size+size, y, 0.0f, 1.0f);
        v2.tex_coords = glm::vec2((message[i]%16)/16.0f + 1/16.0f, (message[i]/16)/16.0f);

        v3.vertices = glm::vec4(x+i*size+size, y+size, 0.0f, 1.0f);
        v3.tex_coords = glm::vec2((message[i]%16)/16.0f + 1/16.0f, (message[i]/16)/16.0f + 1/16.0f);

        v4.vertices = glm::vec4(x+i*size, y+size, 0.0f, 1.0f);
        v4.tex_coords = glm::vec2((message[i]%16)/16.0f, (message[i]/16)/16.0f + 1/16.0f);
